...
class Interpreter:
    ...
    def chat(self, message=None, return_messages=False):
        # Connect to an LLM (large language model)
        if not self.local:
            # gpt-4
            self.verify_api_key()
        ...
        if self.use_azure:
            ...
            if all_env_available:
                ...
                if response == "":
                    ...
                    questions = [
                        inquirer.List('param', message="Parameter count (smaller is faster, larger is more capable)",
        ...
        if not self.local:
            ...
            while attempts < max_attempts:
                ...
                try:
                    ...
                    print(
                        f"Since your LLM API Budget limit was exceeded, you're being switched to local models. Budget: {litellm.max_budget} | Current Cost: {litellm._current_cost}")
